/**
Atomix project, ./test1/main.c, TODO: insert summary here
...
#include <orlib/orlib_sfifos/lib/ipc/ipc_cpy.h>

extern FIFO_Handle f1;
//extern FIFO_Handle f2;

extern FIFO_State f1_state;
...
void doCore1() {
  printf("doCore1\n");
  FIFO_BufferHandle bh, bh_dst, bh_src;
  int i;
  ...
  for (i = 0; i < N_RW_ITERATIONS; i++) {
    ...
    bh = FIFO_getNextWriteBuffer(f1);

    //produce data and put it into the fifo F1
    doA(bh);

    //declare write done
    FIFO_writeDone(f1, bh);

    //core1: T
    //setup data transfer from F1 to F2
    bh_dst = FIFO_getNextWriteBuffer(FIFO_getFifoRemoteHandle(f1));
    bh_src = FIFO_getNextReadBuffer(f1);

    doT(bh_dst, bh_src);

    FIFO_writeDone(FIFO_getFifoRemoteHandle(f1), bh_dst);
    FIFO_readDone(f1, bh_src);
  }
}
